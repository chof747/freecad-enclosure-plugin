...
**Independent Test**: In a clean document, run the toolbar action, enter or accept
default values for length, width, height, wall thickness, and gap, and verify both
body and lid are created and can be regenerated by changing parameters.

**Acceptance Scenarios**:

1. **Given** an open FreeCAD document, **When** the user runs the enclosure toolbar
   action with valid parameters, **Then** the system creates one hollow body and one
   matching lid in the document.
